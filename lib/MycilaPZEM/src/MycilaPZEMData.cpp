...
float Mycila::PZEM::Data::resistance() const {
  std::lock_guard<std::mutex> lock(_dataMutex);
  return current == 0 ? NAN : std::abs(activePower / (current * current));
}

float Mycila::PZEM::Data::dimmedVoltage() const {
  std::lock_guard<std::mutex> lock(_dataMutex);
  return current == 0 ? NAN : std::abs(activePower / current);
}

float Mycila::PZEM::Data::nominalPower() const {
  std::lock_guard<std::mutex> lock(_dataMutex);
  return activePower == 0 ? NAN : std::abs(voltage * voltage * current * current / activePower);
}
...
void Mycila::PZEM::Data::toJson(const JsonObject& root) const {
  ...
  float r = current == 0 ? NAN : std::abs(activePower / (current * current));
  float d = current == 0 ? NAN : std::abs(activePower / current);
  float n = activePower == 0 ? NAN : std::abs(voltage * voltage * current * current / activePower);
  float t = NAN;
  if (powerFactor != 0) {
    const float cosPhi = std::cos(0 * DEG_TO_RAD_F); // phi = 0
    t = 100.0f * std::sqrt((cosPhi * cosPhi) / (powerFactor * powerFactor) - 1);
  }
