...
void setup()
{
  ...
  mqttcommand->setOnSubscribe([&](uint16_t packetid, uint8_t qos)
                              { Serial.println("MQTT_Command:Successful subscription."); });
  mqttcommand->setOnUnsubscribe([&](uint16_t packetid)
                                { Serial.println("MQTT_Command:Successful unsubscription."); });
  mqttcommand->setOnDisconnect([&](AsyncMqttClientDisconnectReason reason)
                               { Serial.println("MQTT_Command: Disconnected. Reason: " + String(uint8_t(reason))); });
